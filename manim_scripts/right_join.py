...
class RightJoin(Scene):
    def construct(self):

        title = Text("Right Join", font = "Rubik", weight = "BOLD")
        title.scale(1.2).set_color(joins_palette_hex["whitedwarf"])

        description = Text(
            """
            •  All data from right is kept
        ...
        self.add(title.to_corner(LEFT + UP), right_circle, description.to_edge(RIGHT, 0.8)).wait(1)
        self.play(FadeIn(left_circle))
        self.play(left_circle.animate.move_to(LEFT * 3.5).set_fill(opacity = 0.8), run_time = 2)
        ...
        self.wait(1)
        self.play(FadeOut(left_circle))

        final_circle = VGroup(right_circle, join)

        self.play(final_circle.animate.move_to(LEFT * 2.5).scale(1.3).set_stroke(width = 10))

        self.wait(5)


        # EXAMPLE TABLES ANIMATION
        self.next_section("Right Join Example")
        self.clear()
        ...
        right_3 = Table(
            ...
            include_background_rectangle=False,
            background_rectangle_color = joins_palette_hex["supernova"],
            line_config={"color": joins_palette_hex["supernova"], "stroke_width": 7}).scale(0.8)
        ...
        right_3.add_highlighted_cell((2,4), joins_palette_hex["aurora"])
        
        right_3.add_highlighted_cell((2,5), joins_palette_hex["aurora"])
        ...
        self.play(TransformMatchingShapes(left_2, left_3))
        
        self.wait(2)
        
        self.play(TransformMatchingShapes(left_3, right_3), TransformMatchingShapes(right_2, right_3))
